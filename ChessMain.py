...
def main():
    ...
    player_one = (
        False  # if a human is playing white, then this will be True, else False
    )
    player_two = (
    ...
    while running:
        ...
        for e in p.event.get():
            ...
            # mouse handler
                ...
                if not game_over:
                    ...
                    if (
                        square_selected == (row, col) or col >= 6
        ...
        if not game_over and not human_turn and not move_undone:
            ...
            if not move_finder_process.is_alive():
                ...
                if ai_move is None or (
                    len(game_state.move_log) >= 3
                    and game_state.move_log[-1] == game_state.move_log[-3]
                ):
                    ai_move = ChessAI.findRandomMove(valid_moves)

                print(returnStatte(game_state.board, ai_move))
...
def drawBoard(screen):
    ...
    """
    global colors
    colors = [p.Color("#f2e1c3"), p.Color("#c3a082")]
...
def returnStatte(state1):

    neww = ChessEngine.GameState()
    neww.board = state1
    ...
    if not move_finder_process.is_alive():
        ...
        returnStatte(game_state.board, ai_move)
        # game_state.makeMove(ai_move)
    # print(game_state.board)
    move_made = True
    animate = True
