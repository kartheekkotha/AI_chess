...
class GameState:
    ...
    def makeMove(self, move):
        ...
        # castle move
        # if move.end_col < 5 and move.is_castle_move:
    ...
    #         elif move.end_col == 3:  # right rook
    ...
    #                 self.current_castling_rights.wqs = False
    #             elif move.start_col == 5:  # right rook
    #                 self.current_castling_rights.wks = False
    #     elif move.piece_moved == "bR":
    #         if move.start_row == 0:
    #             if move.start_col == 0:  # left rook
    #                 self.current_castling_rights.bqs = False
    #             elif move.start_col == 3:  # right rook
    #                 self.current_castling_rights.bks = False
    ...
    def getHash(self):
        # Convert the relevant game state information into a tuple for hashing
        hashable_state = (
            tuple(map(tuple, self.board)),
            self.white_to_move,
            # Add any other relevant information to ensure a unique hash
        )
    ...
    def getPawnMoves(self, row, col, moves):
        ...
        if col - 1 >= 0:  # capture to the left
            if not piece_pinned or pin_direction == (move_amount, -1):
                ...
                #             )
                #         )
        if col + 1 <= 5:  # capture to the right
            if not piece_pinned or pin_direction == (move_amount, +1):
                if self.board[row + move_amount][col + 1][0] == enemy_color:
                    moves.append(
                        Move((row, col), (row + move_amount, col + 1), self.board)
...
class Move:
    ...
    ranks_to_rows = {"1": 5, "2": 4, "3": 3, "4": 2, "5": 1, "6": 0}
    rows_to_ranks = {v: k for k, v in ranks_to_rows.items()}
    files_to_cols = {"a": 0, "b": 1, "c": 2, "d": 3, "e": 4, "f": 5}
